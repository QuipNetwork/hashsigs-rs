// The profile-independent half of @quip.network/hashsigs-wasm.
//
// Every wasm binary this package ships exports the same names with the same
// shapes, so all of the decompose/recompose wiring, the key types, and the
// noble-style namespace objects are written once here and bound to a profile's
// module by `makeHashSigs`. The per-profile entry points under `profiles/` are
// generated (see `scripts/gen-profile-entries.mjs`) and hold nothing but the
// loader wiring and the profile name.
//
// `ShrincsWasmModule` is typed from the default profile's generated `.d.ts`.
// The six generated modules are structurally identical -- one Rust macro emits
// all of them (`src/wasm/export.rs`) -- so a module from any profile satisfies
// this type. What differs between binaries is which profile the exported
// functions compute under, which no type can express; call `profileName()` to
// read it at runtime.
...
// exports condition maps a profile entry to its loader.browser.js, which has
...
} from "./profiles/256s-keccak/nodejs/hashsigs_rs.js";
...
/** The `sphincsPlusC` namespace object a profile entry's `loadHashSigs()` resolves to. */
export type SphincsPlusC = ReturnType<typeof makeSphincsPlusC>;
/** The `shrincs` namespace object a profile entry's `loadHashSigs()` resolves to. */
export type Shrincs = ReturnType<typeof makeShrincs>;

/** The bundle a profile entry's `loadHashSigs()` resolves to. */
export interface HashSigs {
  sphincsPlusC: SphincsPlusC;
  shrincs: Shrincs;
  shrincsImportSigningKey: (secretKey: Uint8Array) => ShrincsKeys;
  /**
   * The SHRINCS profile the loaded binary carries, read from the wasm rather
   * than from the import path. Every profile binary exports identical names,
   * so this is the only way to confirm which one is running.
   */
  profileName: string;
}

/**
 * Assemble the noble-style surface on top of an already-loaded wasm module.
 * The per-profile entry points call this; consumers reach it through their
 * profile's `loadHashSigs()`.
 *
 * ```ts
 * import { loadHashSigs } from "@quip.network/hashsigs-wasm/128s-q18";
 ...
 * ```
 */
export function makeHashSigs(wasm: ShrincsWasmModule): HashSigs {
  return {
