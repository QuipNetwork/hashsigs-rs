...
//! The `shrincs-128s-q20-sha2` profile. Mirrors
//! `contracts/profiles/128s-q20/SHRINCSParams.sol`. Every numeric parameter is
//! identical to [`crate::profiles::p128s_q20`]; only the scheme hash suite and
//! the profile identity string differ. It differs from
//! [`crate::profiles::p128s_q18_sha2`] only in `STATELESS_SIGNATURE_LIMIT`.

use crate::hash::suite::Sha2256Suite;
use crate::profile::Profile;
use crate::profiles::identity;

/// Parameter tuple for `shrincs-128s-q20-sha2`.
pub struct Profile128sQ20Sha2;

impl Profile for Profile128sQ20Sha2 {
    type Suite = Sha2256Suite;
    const PROFILE_NAME: &'static str = "shrincs-128s-q20-sha2";
    const PROFILE_ID: [u8; 32] = identity::p128s_q20_sha2::PROFILE_ID;
...
const _: () = assert!(
    crate::profile::str_eq(
        <Profile128sQ20Sha2 as Profile>::PROFILE_NAME,
        identity::p128s_q20_sha2::PROFILE_NAME
    ),
    "profile type PROFILE_NAME disagrees with the build script's profile identity"
);

/// `shrincs-128s-q20-sha2` instantiated.
pub type Shrincs = crate::shrincs::ShrincsCore<Profile128sQ20Sha2, 32, 1>;
