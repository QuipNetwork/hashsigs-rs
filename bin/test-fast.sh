...
usage() {
  cat <<'EOF'
Usage: ./bin/test-fast.sh <area> [extra cargo args...]
...
case "$area" in
  ...
  compile-default)
    ...
    ;;
  signer)
    cargo test shrincs::signers::shrincs_signer "$@"
    ;;
  signer-stateful)
    cargo test generated_stateful_signature_verifies -- --exact "$@"
    ;;
  signer-import)
    cargo test import_ -- --nocapture "$@"
    ;;
  signer-import-exact)
    if [ "${1:-}" = "" ]; then
    ...
    ;;
  signer-boundary)
    cargo test stateful_boundary_leaves_and_empty_message_round_trip -- --exact "$@"
    ;;
  signer-stateless)
    cargo test generated_stateless_raw_signature_verifies -- --exact "$@"
    ;;
  signer-exact)
    if [ "${1:-}" = "" ]; then
  ...
  vectors)
    ...
    ;;
  vectors-shrincs)
    cargo test --test test_vectors solidity_exported_ "$@"
    ;;
  vectors-exact)
    if [ "${1:-}" = "" ]; then
    ...
    test_name="$1"
    shift
    cargo test --test test_vectors "$test_name" -- --exact "$@"
    ;;
  solidity)
  ...
  solidity-exact)
    ...
    test_name="$1"
    shift
    cargo test --test solidity_account_vectors "$test_name" -- --exact "$@"
    ;;
  wasm)
    cargo test wasm:: -- --nocapture "$@"
    ;;
  wasm-exact)
    if [ "${1:-}" = "" ]; then
    ...
    ;;
  wasm-compile)
    cargo test --features wasm-bindings --target wasm32-unknown-unknown --no-run "$@"
    ;;
  wasm-node)
