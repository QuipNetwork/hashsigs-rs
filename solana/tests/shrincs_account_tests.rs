...
async fn setup_test() -> (ProgramTest, Keypair) {
    ...
    // NOTE: no set_compute_max_units here -- that override replaces the
    // whole per-transaction compute budget in solana-program-test and
    // silently discards the ComputeBudgetInstruction::request_heap_frame
    // each test transaction carries. Budget is requested per-transaction
    // in `send` instead.
...
async fn send(
    ...
    // Stateless-signature-carrying payloads (~30 KB Borsh at 256s) exceed the
    // default 32 KiB program heap under the SBF VM; production callers must
    // request a larger heap frame the same way.
