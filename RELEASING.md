...
Every stub release is a prerelease, so default pip resolution never picks
it. Later plans add the real Python API, the npm profile siblings, and
the C tarballs.

**Know what each registry lets you undo.** npm lets you unpublish a version
within 72 hours. PyPI lets you delete a release, but the filename stays
reserved permanently. Crates.io lets you yank a version and nothing more.
It never lets you replace or re-upload a version.

`publish-crates` is automatic by design. It runs only after `tag-gate` and
`release:validate` pass on the tag. After it runs, you cannot use that
version number again. The npm release stays staged until a maintainer
approves it, so npm can go live after crates.io has already published.

## One-time prerequisites

**The project stores no registry tokens.** `publish-npm` authenticates with
...
work. The job checks that the package exists and fails with instructions rather
than a cryptic npm error.
...
1. **Watch `release:validate`.** It runs on merge request pipelines, on the
   default branch, and on every tag. It uses the same build path as the
   publish jobs and fails before it touches any registry. A push to a feature
   branch with no open merge request does not run it. Do not merge a release
   branch while `release:validate` is failing on the default branch.
2. **Run the same gate locally** with `make -k check-release`. It needs a Rust
   toolchain, the `wasm32-unknown-unknown` target, the `wasm-bindgen`
   command-line tool at the version `Cargo.toml` pins, and `node` with
   `npm`. The npm leg installs its own JavaScript dependencies.
3. **Check the version.** Every manifest must carry the version you are
   tagging. `make check-versions` proves it.
...
The version determines the dist-tag, which is not always `latest`. A plain
`X.Y.Z` stages on `latest`. A prerelease stages on the letters of its
prerelease identifier, so `0.2.0-rc.2` goes to `rc` and `0.3.0-beta.1` goes to
`beta`. This rule keeps `npm install @quip.network/hashsigs-wasm` from
picking up a release candidate.

**The crate and the wheel need no approval step.** `publish-crates` and
`publish-pypi` publish immediately after the gates pass on the tag. Confirm
the version appears on crates.io and PyPI. If you must pull back a crate
release, `cargo yank` is the only tool, and the version number stays spent.
You can delete a PyPI release, but its filenames stay reserved.

## When you cut a tag against a red tree

You lose nothing and nothing leaks. The tag pipeline runs `release:validate`
again ahead of every publish job, so it fails at the same step and touches
no registry. Delete the tag, fix the cause, and re-cut.
